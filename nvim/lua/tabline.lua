...
vim.highlight.create('SeparatorBlue', { guibg = bg, guifg = blue, gui = 'bold' }, false)
vim.highlight.create('SeparatorGrey', { guibg = bg, guifg = grey, gui = 'bold' }, false)
...
local function Tabline()
    ...
    for k, v in ipairs(buf_listed) do
        ...
        if (vim.fn.bufnr('%') == vim.fn.bufnr(v)) then
            ...
            buf_name = buf_name .. active_buf_hi .. targets[k] .. ": " .. vim.fn.bufname(v)
            buf_name = buf_name .. blue_sep_hi .. ' '
            buf_name = active_buf_hi .. buf_name .. inactive_buf_hi
        else
            buf_name = grey_sep_hi .. ''
            buf_name = buf_name .. inactive_buf_hi .. targets[k] .. ": " .. vim.fn.bufname(v)
